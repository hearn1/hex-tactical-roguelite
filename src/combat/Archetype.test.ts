...
describe("Archetype: Roll Save", () => {
  it("rollSave returns true when d20 + mod >= DC", () => {
    ...
    const unit = makeUnit({ instanceId: "u", pos: { q: 0, r: 0 }, stats: { maxHp: 10, armor: 10, move: 3, str: 0, dex: 0, con: 0, int: 3, wis: 0, cha: 0 } });
    // int=3, d20=10 → 13 >= 12
    expect(rollSave(unit, "int", 12, controlledRng)).toBe(true);
    // int=3, d20=10 → 13 < 15
    expect(rollSave(unit, "int", 15, controlledRng)).toBe(false);
  });
});
...
describe("Archetype: Protective Ward (Cloistered)", () => {
  it("applies warded condition granting +3 effective armor", () => {
    ...
    const caster = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, defId: "class.acolyte", stats: { maxHp: 14, armor: 12, move: 3, str: 1, dex: 1, con: 0, int: 3, wis: 0, cha: 0 } });
    const ally = makeUnit({ instanceId: "b", pos: { q: 1, r: 0 }, stats: { maxHp: 18, armor: 14, move: 3, str: 3, dex: 1, con: 0, int: 0, wis: 0, cha: 0 } });
...
describe("Archetype: Empowered Spell (Evoker)", () => {
  it("applies empowered condition on ally", () => {
    const rng = createRng(1);
    const evoker = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, defId: "class.arcanist", stats: { maxHp: 11, armor: 11, move: 3, str: 0, dex: 1, con: 0, int: 4, wis: 0, cha: 0 } });
  ...
  it("empowered condition adds 1d6 to next damaging spell", () => {
    ...
    const evoker = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, defId: "class.arcanist", stats: { maxHp: 100, armor: 10, move: 3, str: 0, dex: 1, con: 0, int: 10, wis: 0, cha: 0 } });
    const target = makeUnit({ instanceId: "t", pos: { q: 1, r: 0 }, team: "enemy", hp: 100, stats: { maxHp: 100, armor: 5, move: 3, str: 0, dex: 0, con: 0, int: 0, wis: 0, cha: 0 } });
    const state = twoUnitState(evoker, target);

    // Without empowered
    const plainTarget = makeUnit({ instanceId: "t2", pos: { q: 1, r: 0 }, team: "enemy", hp: 100, stats: { maxHp: 100, armor: 5, move: 3, str: 0, dex: 0, con: 0, int: 0, wis: 0, cha: 0 } });
...
describe("Archetype: Healing Burst (Beacon) AoE", () => {
  it("heals allies within radius 2", () => {
    const rng = createRng(1);
    const beacon = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, defId: "class.acolyte", stats: { maxHp: 14, armor: 12, move: 3, str: 1, dex: 1, con: 0, int: 5, wis: 0, cha: 0 } });
    const ally1 = makeUnit({ instanceId: "b", pos: { q: 1, r: 0 }, hp: 5, stats: { maxHp: 18, armor: 14, move: 3, str: 3, dex: 1, con: 0, int: 0, wis: 0, cha: 0 } });
    const ally2 = makeUnit({ instanceId: "c", pos: { q: 0, r: 1 }, hp: 5, stats: { maxHp: 14, armor: 12, move: 3, str: 1, dex: 1, con: 0, int: 3, wis: 0, cha: 0 } });
    const allyFar = makeUnit({ instanceId: "d", pos: { q: 0, r: 3 }, hp: 5, stats: { maxHp: 14, armor: 12, move: 3, str: 1, dex: 1, con: 0, int: 3, wis: 0, cha: 0 } });
    ...
    // allyFar is at distance 3 from beacon (0,0) â†’ (0,3) = 3, out of radius 2
...
describe("Archetype: Taunt (Shieldbearer)", () => {
  it("applies taunted condition and sets forcedTargetId on enemy", () => {
    const rng = createRng(1);
    const guardian = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, stats: { maxHp: 18, armor: 14, move: 3, str: 3, dex: 1, con: 0, int: 0, wis: 0, cha: 0 } });
    const enemy = makeUnit({ instanceId: "e", pos: { q: 1, r: 0 }, team: "enemy", stats: { maxHp: 10, armor: 12, move: 3, str: 1, dex: 2, con: 0, int: 0, wis: 0, cha: 0 } });
...
describe("Archetype: Mesmerize (Enchanter) with save", () => {
  it("applies mesmerized condition on failed save, negated on success", () => {
    ...
    const enchanter = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, defId: "class.arcanist", stats: { maxHp: 11, armor: 11, move: 3, str: 0, dex: 1, con: 0, int: 4, wis: 0, cha: 0 } });
    ...
    callIdx = 0;
    const enemyNoSave = makeUnit({ instanceId: "e2", pos: { q: 1, r: 0 }, team: "enemy", hp: 20, stats: { maxHp: 20, armor: 12, move: 3, str: 0, dex: 0, con: 0, int: 0, wis: 0, cha: 0 } });
...
describe("Archetype: Retributive Strike (Vindicator reaction)", () => {
  it("triggers reaction when vindicator is hit by melee attack", () => {
    const rng = createRng(42);
    const vindicator = makeUnit({ instanceId: "v", pos: { q: 0, r: 0 }, team: "hero", hp: 50, stats: { maxHp: 50, armor: 5, move: 3, str: 5, dex: 1, con: 0, int: 0, wis: 0, cha: 0 }, passives: ["archetype_passive.vindicator_below_50_attack"] });
    const enemy = makeUnit({ instanceId: "e", pos: { q: 1, r: 0 }, team: "enemy", hp: 30, stats: { maxHp: 30, armor: 5, move: 3, str: 5, dex: 0, con: 0, int: 0, wis: 0, cha: 0 } });
...
describe("Archetype: Evoker crit floor", () => {
  it("crit on 19-20 with evoker passive", () => {
    ...
    const evoker = makeUnit({ instanceId: "a", pos: { q: 0, r: 0 }, defId: "class.arcanist", stats: { maxHp: 100, armor: 10, move: 3, str: 0, dex: 1, con: 0, int: 10, wis: 0, cha: 0 }, passives: ["archetype_passive.evoker_crit_floor"] });
    // First attack: d20=19 should crit with the passive.
    const target1 = makeUnit({ instanceId: "t1", pos: { q: 1, r: 0 }, team: "enemy", hp: 100, stats: { maxHp: 100, armor: 30, move: 3, str: 0, dex: 0, con: 0, int: 0, wis: 0, cha: 0 } });
